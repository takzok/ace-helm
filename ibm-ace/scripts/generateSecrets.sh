...
######################################################
# Create the configuration Kubernetes secret resource
######################################################
...
######################################################
# Create the users Kubernetes secret resource
######################################################

if [ $# -eq 2 ]; then

  TARGET_USERS_SECRET_NAME="${RELEASE_NAME}-ibm-ace"
  
  # Build up the arguments for the kubectl command
  SECRET_ARGS=

  if [ -s ./adminusers.txt ]; then
    SECRET_ARGS="${SECRET_ARGS} --from-file=adminusers=./adminusers.txt"
  fi

  if [ -s ./viewerusers.txt ]; then
    SECRET_ARGS="${SECRET_ARGS} --from-file=viewerusers=./viewerusers.txt"
  fi

  # Create the Kubernetes secret resource
  echo "Creating users secret"
  echo "kubectl create secret generic ${TARGET_USERS_SECRET_NAME}${SECRET_ARGS}"
  kubectl create secret generic ${TARGET_USERS_SECRET_NAME}${SECRET_ARGS}
fi
